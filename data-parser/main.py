...
exercisetasks_files = sorted(
    exercisetasks_files, key=lambda x: (x.split(" ")[-1]), reverse=True
)
userdetail_files = sorted(
    userdetail_files, key=lambda x: (x.split(" ")[-1]), reverse=True
)
user_consents_files = sorted(
    user_consents_files, key=lambda x: (x.split(" ")[-1]), reverse=True
)
...
try:
    submissions = (
        pl.read_csv(join("./data/", submission_files[0]))
...
try:
    user_details = pl.read_csv(join("./data/", userdetail_files[0])).select(
        [
            "user_id",
            pl.concat_str(
                pl.col("^.*_name$").fill_null("").alias("name"), separator=" "
            ),
            "email",
        ]
    )
...
try:
    user_consents = pl.read_csv(join("./data/", user_consents_files[0])).select(
        ["course_id", "question", "user_id", "research_consent"]
    )

except OSError as error:
    print(error)

exercise_tasks = exercise_tasks.sort(["course_module_name", "exercise_name"])

cleaned_subs = (
    submissions.join(
        user_details.select(pl.exclude("created_at")), on="user_id", how="left"
    )
    .join(
        exercise_tasks.select(
            ["id", "exercise_type", "exercise_name", "course_module_name"]
        ),
        left_on="exercise_task_id",
        right_on="id",
        how="left",
    )
    ...
    .sort("course_module_name", "exercise_name")
)
user_consents = user_consents.with_columns(
    pl.col("question")
    .str.replace_all(",", "")
    .str.replace("Hyväksyn että ", "")
    .str.slice(0, 40)
)

user_consents = user_consents.pivot(
    index="user_id",
    columns="question",
...
exercise_tasks_map = dict(
    [
        (x[0], json.loads(x[1]))
        for x in exercise_tasks.select(["id", "private_spec", "exercise_type"]).rows()
        if "factorial" in x[2]
    ]
)
...
for k, v in exercise_tasks_map.items():
    ...
    # }
    if v["type"] == "factorial":
        exercise_tasks_map[k] = dict(
            [
                (key, val)
                for key, val in dict(v).items()
                if key in ["id", "type", "questions", "options"]
            ]
        )
        exercise_tasks_map[k]["questions"] = [
            dict(
                [
                    (key, val)
                    for key, val in dict(quest).items()
                    if key not in ["question", "mandatory"]
                ]
            )
            for quest in exercise_tasks_map[k]["questions"]
            if quest["questionLabel"] not in "info"
        ]
        exercise_tasks_map[k]["exercise_name"] = exercise_tasks.select(
            ["exercise_name", "id"]
        ).row(by_predicate=(pl.col("id") == k))[0]
        exercise_tasks_map[k]["course_module_name"] = exercise_tasks.select(
            ["course_module_name", "id"]
        ).row(by_predicate=(pl.col("id") == k))[0]
    ...
    # }
        ...
        exercise_tasks_map[k] = dict(
            [
                (un, val)
                for un, val in dict(v).items()
                if un in ["id", "type", "content"]
            ]
        )
        ...
        content = [
            dict(
                [
                    ...
                    if key in ["id", "answer", "question"]
                ]
            )
            for item in content
            if item["question"]["questionLabel"] not in ["info"]
        ]
...
for k, v in exercise_tasks_map.items():
    submissions_data = (
        cleaned_subs.filter(pl.col("exercise_task_id") == k)
        .select("user_id", "data_json", "created_at")
        .rows()
    )
    ...
    if v["type"] == "factorial":
        ...
        typed_col_labels = dict(
            ChainMap(*[{lab["questionLabel"]: pl.Int8} for lab in v["questions"]])
        )
        ...
        for row in submissions_data:
            ...
            submission = dict(
                ChainMap(
                    *[
                        {item["questionLabel"]: options.get(item.get("chosenOptionId"))}
                        for item in user_answers.get("answeredQuestions")
                    ]
                )
            )
            ...
            exercise_name_label_for_timestamp = v["exercise_name"]
            if (
                exercise_name_label_for_timestamp
                not in module_user_details[ind].columns
            ):
                submission[exercise_name_label_for_timestamp] = row[2][
                    0:19
                ]  # create_at field
        ...
        col_labels = [
            (
                [
                    " ".join([item["questionLabel"], option.split(";", 1)[0].strip()])
                    for option in item["options"]
                ]
                if item["answer-type"] == "multiple-choice"
                else item["questionLabel"]
            )
        ...
        for row in submissions_data:
            ...
            user_answer = dict(
                [
                    (answeredQ.get("questionLabel"), answeredQ.get("answer"))
                    for answeredQ in user_answer.get("answeredQuestions")
                ]
            )
    ...
    module_user_details[ind] = module_user_details[ind].join(
        additional_cols, how="left", on=["user_id", "course_module_name"]
    )
...
user_details.write_csv(
    filename, has_header=True, quote='"', null_value="", separator=";"
)
