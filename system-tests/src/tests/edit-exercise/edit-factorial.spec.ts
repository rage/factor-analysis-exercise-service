...
test("can add factorial survey type wtih information elements and questions intermixed", async ({
  ...
  await page.frameLocator("iframe").locator("text=Add Option").click()
  // Fill [id="option-text-1"]
  await page.frameLocator("iframe").locator('[id="option-text-1"]').fill("no")
  // Click [id="rate-value-1"]
  await page.frameLocator("iframe").locator('[id="rate-value-1"]').click()
  // Fill [id="rate-value-1"]
  await page.frameLocator("iframe").locator('[id="rate-value-1"]').fill("1")
  // Click text=Add Option
  await page.frameLocator("iframe").locator("text=Add Option").click()
  // Click [id="option-text-2"]
  await page.frameLocator("iframe").locator('[id="option-text-2"]').click()
  // Fill [id="option-text-2"]
  await page.frameLocator("iframe").locator('[id="option-text-2"]').fill("maybe")
  // Click [id="rate-value-2"]
  await page.frameLocator("iframe").locator('[id="rate-value-2"]').click()
  // Fill [id="rate-value-2"]
  await page.frameLocator("iframe").locator('[id="rate-value-2"]').fill("2")
  // Click text=Add Option
  await page.frameLocator("iframe").locator("text=Add Option").click()
  // Click [id="option-text-3"]
  await page.frameLocator("iframe").locator('[id="option-text-3"]').click()
  // Fill [id="option-text-3"]
  await page.frameLocator("iframe").locator('[id="option-text-3"]').fill("yes")
  // Click [id="rate-value-3"]
  await page.frameLocator("iframe").locator('[id="rate-value-3"]').click()
  // Fill [id="rate-value-3"]
  await page.frameLocator("iframe").locator('[id="rate-value-3"]').fill("3")
  // Click text=Add Option
  await page.frameLocator("iframe").locator("text=Add Option").click()
  // Click [id="option-text-4"]
  await page.frameLocator("iframe").locator('[id="option-text-4"]').click()
  // Fill [id="option-text-4"]
  await page.frameLocator("iframe").locator('[id="option-text-4"]').fill("Can't tell")
  // Click [id="rate-value-4"]
  await page.frameLocator("iframe").locator('[id="rate-value-4"]').click()
  // Fill [id="rate-value-4"]
  await page.frameLocator("iframe").locator('[id="rate-value-4"]').fill("")
